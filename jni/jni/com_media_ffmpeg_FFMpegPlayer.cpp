...
static void process_media_player_call(JNIEnv *env, jobject thiz, status_t opStatus, const char* exception, const char *message)
{
    if (exception == NULL) {  // Don't throw exception. Instead, send an event.
    	__android_log_print(ANDROID_LOG_INFO, TAG, "exception == null and do nothing !");
		/*
		...
		*/
        ...
        if ( opStatus == (status_t) INVALID_OPERATION ) {
            ...
            jniThrowException(env, "java/lang/IllegalStateException", "player's status can't be operated! throw illegalstateException!");
            __android_log_print(ANDROID_LOG_INFO, TAG, "player's status can't be operated! throw illegalstateException!--->status:0x%X",opStatus);
        } else if ( opStatus != (status_t) OK ) {
            if (strlen(message) > 230) {
               // if the message is too long, don't bother displaying the status code
               jniThrowException( env, exception, message);
               __android_log_print(ANDROID_LOG_INFO, TAG, " throw exception:%s and the msg is beyond 230char",exception);
            } else {
               char msg[256];
                // append the status code to the message
               sprintf(msg, "%s: status=0x%X", message, opStatus);
               jniThrowException( env, exception, msg);
               __android_log_print(ANDROID_LOG_INFO, TAG, "%s: status=0x%X", message, opStatus);
            }
        }
    }
}
