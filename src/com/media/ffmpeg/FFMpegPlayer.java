...
public class FFMpegPlayer {
	 ...
	 */
	/*
	 * public void setWakeMode(Context context, int mode) { boolean washeld =
	 * false; if (mWakeLock != null) { if (mWakeLock.isHeld()) { washeld = true;
	 * mWakeLock.release(); } mWakeLock = null; }
	 * 
	 * PowerManager pm =
	 * (PowerManager)context.getSystemService(Context.POWER_SERVICE); mWakeLock
	 * = pm.newWakeLock(mode|PowerManager.ON_AFTER_RELEASE,
	 * FFMpegPlayer.class.getName()); mWakeLock.setReferenceCounted(false); if
	 * (washeld) { mWakeLock.acquire(); } }
	 */
	...
	/*
	 * private void stayAwake(boolean awake) { if (mWakeLock != null) { if
	 * (awake && !mWakeLock.isHeld()) { mWakeLock.acquire(); } else if (!awake
	 * && mWakeLock.isHeld()) { mWakeLock.release(); } } mStayAwake = awake;
	 * updateSurfaceScreenOn(); }
	 */
	 ...
	 * 
	 * @param path
	 *            the path of the file you want to play
	 * @throws IllegalStateException
	 *             if it is called in an invalid state
	 */
